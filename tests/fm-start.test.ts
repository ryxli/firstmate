...
`);
	...
	return { temp, home, state, bin, output: join(temp, "launch") };
}
...
function startFm(fx: Fixture, home: string | undefined, extra: NodeJS.ProcessEnv = {}): StartedFm {
	let stdout = "";
	let stderr = "";
	const child = spawn(FM, ["start"], {
		cwd: REPO_ROOT,
...
async function runFm(fx: Fixture, home: string | undefined, extra: NodeJS.ProcessEnv = {}): Promise<{ status: number; stdout: string; stderr: string }> {
	const started = startFm(fx, home, extra);
...
function readLaunch(output: string): LaunchResult {
	return {
		...
		pid: readFileSync(`${output}.pid`, "utf8"),
		lock: readFileSync(`${output}.lock`, "utf8"),
	};
}
...
describe("fm start prompt", () => {
	it("uses the normal kickoff and hides handoff mechanics from the model", async () => {
		const fx = fixture();
		...
		expect(result.kickoff).not.toContain("lock");
		expect(result.kickoff).not.toContain("handoff");
		...
		expect(result.appendSystemPrompt).not.toContain("Do not reacquire it during startup");
		expect(result.appendSystemPrompt).not.toContain("sbin/fm lock` to acquire");
	});
});
