// Admission-control: capContextBlock, mainPreloadBlock, identity name bound.
...
import {
	CAP_CONTEXT_MAX_BYTES,
	CapContextOversizeError,
...
describe("normalizeCapPayload", () => {
	it("keeps one terminal newline and preserves leading whitespace", () => {
		expect(normalizeCapPayload("  hello\n\n\n")).toBe("  hello\n");
		expect(normalizeCapPayload("hello")).toBe("hello\n");
	});
});

describe("capContextBlock / mainPreloadBlock", () => {
	it("omits cap when missing and returns stub only", () => {
		const home = tempHome();
		try {
			expect(capContextBlock(home)).toBeNull();
	...
	it("admits compact cap and preserves terminal newline in preload join", () => {
		const home = tempHome();
		const body = "# Cap preferences\n\nTerse.\n";
		writeFileSync(join(home, "data", "cap.md"), body);
		try {
			const admitted = capContextBlock(home);
			expect(admitted).toBe(normalizeCapPayload(body));
			...
			expect(preload).toContain("# Cap preferences");
			expect(preload).not.toContain("# Preloaded fleet registries");
	...
	it("does not admit planted registry sentinels from projects/secondmates files", () => {
		const home = tempHome();
		writeFileSync(join(home, "data", "cap.md"), "# Cap preferences\nok\n");
		writeFileSync(
	...
	it("admits exactly at the cap byte ceiling", () => {
		const home = tempHome();
		const prefix = "# Cap preferences\n\n";
		const pad = "x".repeat(CAP_CONTEXT_MAX_BYTES - Buffer.byteLength(prefix, "utf8") - 1);
		const raw = `${prefix}${pad}`;
		const admitted = normalizeCapPayload(raw);
		expect(Buffer.byteLength(admitted, "utf8")).toBe(CAP_CONTEXT_MAX_BYTES);
		writeFileSync(join(home, "data", "cap.md"), raw);
		try {
			expect(capContextBlock(home)).toBe(admitted);
		} finally {
			rmSync(home, { recursive: true, force: true });
		}
	});

	it("fails closed one byte over the cap ceiling and never truncates", () => {
		const home = tempHome();
		const path = join(home, "data", "cap.md");
		const prefix = "# Cap preferences\n\n";
		const pad = "x".repeat(CAP_CONTEXT_MAX_BYTES - Buffer.byteLength(prefix, "utf8"));
		const raw = `${prefix}${pad}`;
		expect(Buffer.byteLength(normalizeCapPayload(raw), "utf8")).toBe(CAP_CONTEXT_MAX_BYTES + 1);
		writeFileSync(path, raw);
		try {
			expect(() => capContextBlock(home)).toThrow(CapContextOversizeError);
			try {
				capContextBlock(home);
			} catch (error) {
				const err = error as CapContextOversizeError;
				expect(err.path).toBe(path);
				expect(err.actualBytes).toBe(CAP_CONTEXT_MAX_BYTES + 1);
				expect(err.maxBytes).toBe(CAP_CONTEXT_MAX_BYTES);
			}
			expect(() => mainPreloadBlock(home)).toThrow(CapContextOversizeError);
		} finally {
			rmSync(home, { recursive: true, force: true });
		}
	});

	it("counts multibyte UTF-8 at the cap ceiling without truncating", () => {
		const home = tempHome();
		const prefix = "# Cap preferences\n\n";
		const budget = CAP_CONTEXT_MAX_BYTES - Buffer.byteLength(prefix, "utf8") - 1;
		const chars = Math.floor(budget / 2);
		const raw = `${prefix}${"é".repeat(chars)}`;
		const admitted = normalizeCapPayload(raw);
		expect(Buffer.byteLength(admitted, "utf8")).toBeLessThanOrEqual(CAP_CONTEXT_MAX_BYTES);
		writeFileSync(join(home, "data", "cap.md"), raw);
		try {
			expect(capContextBlock(home)).toBe(admitted);
	...
	it("fails closed when data/cap.md is unreadable for a non-ENOENT reason", () => {
		...
		mkdirSync(path);
		try {
			expect(() => capContextBlock(home)).toThrow();
			try {
				capContextBlock(home);
			} catch (error) {
...
describe("identity display name bound", () => {
	...
	it("role contract at max name stays within the main/unverified byte ratchet", () => {
		...
		try {
			...
			expect(Buffer.byteLength(contract, "utf8")).toBe(505);
		} finally {
			rmSync(home, { recursive: true, force: true });
		}
	});
