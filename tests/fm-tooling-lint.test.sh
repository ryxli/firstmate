...
#   - `npx` (whole word), `node <path-with-dist>`, and a raw `./bin/*.js`
...
GUARD="$ROOT/bin/fm-tooling-lint.sh"
...
fixture() {
  local d="$TMP_ROOT/$1"
  mkdir -p "$d/bin" "$d/.agents/skills/demo"
  printf '%s\n' "$d"
}
...
test_binjs_fails() {
  ...
  printf 'Also `./bin/tool.js run` works.\n' > "$d/README.md"
  OUT=$("$GUARD" "$d" 2>&1); RC=$?
  [ "$RC" -eq 1 ] || fail "bin/*.js should fail (rc=$RC): $OUT"
  printf '%s\n' "$OUT" | grep -qi '.js script' || fail "did not report .js script: $OUT"
  pass "raw ./bin/*.js command fails"
}

test_help_text_in_bin_fails() {
  local d; d=$(fixture helptext)
  printf '#!/usr/bin/env bash\necho "run: npx foo"\n' > "$d/bin/demo.sh"
  OUT=$("$GUARD" "$d" 2>&1); RC=$?
  [ "$RC" -eq 1 ] || fail "npx in bin help text should fail (rc=$RC): $OUT"
  printf '%s\n' "$OUT" | grep -q 'demo.sh' || fail "did not name the bin file: $OUT"
  pass "npx in bin/*.sh help text is caught"
}
...
test_definers_exempt() {
  ...
  printf '#!/usr/bin/env bash\n# never npx, never node dist/x, never ./bin/x.js\n' > "$d/bin/fm-brief.sh"
  printf '#!/usr/bin/env bash\n# scans for npx and node dist and ./bin/x.js\n' > "$d/bin/fm-tooling-lint.sh"
  OUT=$("$GUARD" "$d" 2>&1); RC=$?
