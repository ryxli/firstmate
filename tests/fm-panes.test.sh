#!/usr/bin/env bash
# Test the fm panes verb (fleet roster helper).
set -u
...
test_lists_panes_with_agents() {
  local out
  out=$(PATH="$BIN_DIR:$BASE_PATH" "$ROOT/sbin/fm" panes) || fail "script exited non-zero"
...
test_filters_case_insensitive() {
  local out
  out=$(PATH="$BIN_DIR:$BASE_PATH" "$ROOT/sbin/fm" panes "fran") || fail "script exited non-zero"
...
test_filters_uppercase() {
  local out
  out=$(PATH="$BIN_DIR:$BASE_PATH" "$ROOT/sbin/fm" panes "KEEL") || fail "script exited non-zero"
...
test_no_match_returns_empty() {
  local out rc
  out=$(PATH="$BIN_DIR:$BASE_PATH" "$ROOT/sbin/fm" panes "nomatch")
  rc=$?
...
test_herdr_error_exits_nonzero() {
  local rc
  PATH="$BIN_DIR:$BASE_PATH" "$ROOT/sbin/fm" panes 2>/dev/null; rc=$?
...
test_all_flag_shows_agentless_pane() {
  local out
  out=$(PATH="$BIN_DIR:$BASE_PATH" "$ROOT/sbin/fm" panes --all) || fail "script exited non-zero with --all"
...
test_all_flag_output_format_agentless() {
  local out line
  out=$(PATH="$BIN_DIR:$BASE_PATH" "$ROOT/sbin/fm" panes --all) || fail "script exited non-zero"
  line=$(printf '%s\n' "$out" | grep '^-')
