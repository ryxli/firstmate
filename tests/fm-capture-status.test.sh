...
out=$(CAPTURE_EVENTS_PATH="$EVENTS" CAPTURE_LOADED_PATH="$LOADED" "$ROOT/bin/fm-capture-status.sh") || fail "status helper exited non-zero"
...
rm -f "$LOADED"
out=$(CAPTURE_EVENTS_PATH="$EVENTS" CAPTURE_LOADED_PATH="$LOADED" "$ROOT/bin/fm-capture-status.sh") || fail "status helper exited non-zero without marker"
