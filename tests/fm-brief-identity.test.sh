...
test_ship_brief_has_identity_context() {
  ...
  grep -qF 'Your visible tab/agent label: keel/fix-login' "$brief" \
  ...
  pass "ship brief propagates supervisor/chain/workspace/label/report-back"
}
...
test_scout_brief_has_identity_context() {
  ...
  grep -qF 'Your visible tab/agent label: keel/probe' "$brief" \
...
test_brief_identity_defaults_without_config() {
  ...
  grep -qF 'Your visible tab/agent label: fm/fix-x' "$brief" \
