...
# Build a real git project clone under <home>/projects/<proj> with `main` as its
# default branch, left checked out on <branch> (use main for the clean case).
make_git_project() {
  local home=$1 proj=$2 branch=$3 dir
...
test_bootstrap_reports_tangled_primary_checkout() {
  local case_dir fakebin out
  case_dir="$TMP_ROOT/tangle"
  mkdir -p "$case_dir/home"
  fakebin=$(make_fake_toolchain "$case_dir")
  make_git_project "$case_dir/home" myproj feature/escape

  out=$(run_bootstrap "$case_dir/home" "$fakebin")
  printf '%s\n' "$out" \
    | grep -qE '^TANGLE: myproj on feature/escape; restore with: git -C .* checkout main$' \
    || fail "bootstrap did not report the tangled primary checkout: $out"
  pass "bootstrap flags a primary checkout left on a feature branch"
}

test_bootstrap_silent_when_primary_on_default() {
  local case_dir fakebin out
  case_dir="$TMP_ROOT/no-tangle"
  mkdir -p "$case_dir/home"
  fakebin=$(make_fake_toolchain "$case_dir")
  make_git_project "$case_dir/home" myproj main

  out=$(run_bootstrap "$case_dir/home" "$fakebin")
  if printf '%s\n' "$out" | grep -q '^TANGLE:'; then
    fail "bootstrap reported a tangle for a primary checkout on its default branch: $out"
  fi
  pass "bootstrap stays quiet when the primary checkout is on its default branch"
}
