...
Use `bin/fm-home-seed.sh <id> <home|-> [<project>...]` after scaffolding the charter to provision the persistent home and registry entry; `-` creates a herdr-managed git worktree of the firstmate repo at `<code-root>/mates/<id>` (e.g. `~/code/mates/<id>`; a `mates` sibling of the harness dir, matching the cross-ship convention; `FM_HERDR_SM_BASE` overrides the mates base dir) and records the herdr workspace ID in the registry.
...
**Stale.** On a crewmate turn-end the extension arms a stale backstop; it fires only if the pane is still idle past `FM_STALE_ESCALATE_SECS` (default 240s) with no captain-relevant last status. A stale wake directs you to peek the pane (`bin/fm-peek.sh <pane_id>`) to diagnose. Before any stale wake fires the extension confirms the pane still exists and corroborates herdr's idle verdict against the pane's own rendered text (a crew blocked on its own long foreground tool call reads idle to herdr while its busy banner is still up), so it neither wakes on a gone pane nor spuriously flags a still-working one. The periodic stale NAG is SKIPPED for `kind=secondmate` panes (an idle secondmate is healthy - it runs its own supervision) and for ship tasks parked on a green PR (`pr=` set and a terminal `done: PR`/PR-ready status line); those stay covered by the merge `check.sh` and the status stream. A secondmate is not left fully uncovered, though: a secondmate `working->idle` transition (a secondmate finishing routed work and going idle, armed only on the transition so a resting secondmate is never nagged) arms a short, corroborated completion backstop (`FM_SECONDMATE_IDLE_SECS`, default 20s; same target-existence + idle-corroboration + status-log checks) that wakes you when it completed routed work without leaving a terminal captain-relevant status line.

Live pane resolution is durable-identity based: the extension resolves each in-flight task's CURRENT pane from its herdr agent identity (the agent slot is the task id, so `herdr agent get <task>` resolves it) and refreshes a drifted `state/<id>.meta` `pane=` after a restart/reopen, so a state change on a re-identified pane still wakes you.
...
The structured fields above are different: task IDs, blocked-by IDs, and Done-entry PR URLs or report paths are the durable record this schema requires.
Correct or delete stale free-form notes the moment you catch them, and put durable facts in the curated memory homes of section 6, not scattered across one-off task notes.
