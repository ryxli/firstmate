...
Runs on **bun** (like `benchmarks/thinking`): the deterministic core (integrity gates + replay
aggregation) is pure and CI-tested; only the live A/B path spends tokens and is flag-gated out of CI.
Complements `benchmarks/thinking/` (thinking-efficiency A/B) and the deterministic supervision
benchmark: those measure narrow decision rules, this measures end-to-end agentic behavior.
...
`gates.ts` enforces the corpus is fair before any result is trusted; `bench.ts gates` runs them and
...
```
bin/fm-action-bench.sh gates                        # integrity gates only (pure, no tokens) - what CI runs
bin/fm-action-bench.sh replay results/<r>.runs.json # re-aggregate a recorded run (pure)
bin/fm-action-bench.sh run --live --trials 3 --model gpt-5.4-mini --thinking low   # LIVE A/B (costs tokens)
...
gates.ts            the 5 deterministic integrity gates
arms.ts             the arm loader (control = "", harness = arms/harness.txt)
bench.ts            CLI entry: gates | replay | run (guarded by import.meta.main)
scenarios/index.ts  static registry: imports every scenario file, flattens the corpus
scenarios/*.ts      the corpus (easy -> aspirational), each with setup/goal/procedural + oracle + leakMarkers
