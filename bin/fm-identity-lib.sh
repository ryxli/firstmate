...
# a supervisor name, a human-readable task slug, and a worker's visible
# herdr label live in exactly one place and never drift between the
# spawner (which places + names the herdr tab/agent) and the brief
# scaffolder (which propagates the same identity downward in prose).
#
...
# The visible herdr tab/agent label for a crewmate: "<supervisor>/<task-slug>"
# (e.g. "keel/fix-teardown-cleanup"). An explicit label, when given, wins so a
# caller can override the derivation. The random task id is never part of this
# label — it lives only in metadata/backlog/status.
fm_worker_label() {
