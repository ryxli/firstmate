  ...
  if [ -d "$hstate" ]; then
    for f in "$hstate"/*.meta; do
      ...
      if [ "$HERDR_OK" != 1 ]; then
        ...
        live_pid=$(printf '%s' "$pjson" | herdr_json_get result pane pane_id)
        if [ -n "$live_pid" ]; then
          degraded=live
          ws_id=$(printf '%s' "$pjson" | herdr_json_get result pane workspace_id)
          tab_id=$(printf '%s' "$pjson" | herdr_json_get result pane tab_id)
          pane_label=$(printf '%s' "$pjson" | herdr_json_get result pane label)
          cwd=$(printf '%s' "$pjson" | herdr_json_get result pane cwd)
          [ -n "$cwd" ] || cwd=$(printf '%s' "$pjson" | herdr_json_get result pane foreground_cwd)
          ...
          astat=$(printf '%s' "$ajson" | herdr_json_get result agent agent_status)
          [ -n "$astat" ] || astat=$(printf '%s' "$pjson" | herdr_json_get result pane agent_status)
          pane_status="$astat"
        else
      ...
      if [ "$HERDR_OK" = 1 ] && [ -n "$tab_id" ]; then
        ...
        tab_label=$(printf '%s' "$tjson" | herdr_json_get result tab label)
        tab_status=$(printf '%s' "$tjson" | herdr_json_get result tab agent_status)
        [ -n "$ws_id" ] || ws_id=$(printf '%s' "$tjson" | herdr_json_get result tab workspace_id)
      fi
      ...
      if [ "$HERDR_OK" = 1 ] && [ -n "$ws_id" ]; then
        local wjson
        wjson=$(ws_json "$ws_id")
        ws_label=$(printf '%s' "$wjson" | herdr_json_get result workspace label)
        ws_status=$(printf '%s' "$wjson" | herdr_json_get result workspace agent_status)
      fi
