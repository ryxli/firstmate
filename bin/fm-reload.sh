...
# Resolve a target to a concrete herdr pane id, matching fm-peek.sh/fm-send.sh
# (herdr resolves fm_resolve_live_pane's job inline; the durable-identity logic
# lives in the supervision extension, not a bash helper on this base).
resolve() {
  case "$1" in
