...
1. The supervision extension reloads automatically when this session starts and re-resolves the in-flight fleet from `state/*.meta`; there is no wake-queue to drain.
2. Read `data/backlog.md`.
...
4. If the snapshot flags a recorded direct-report pane missing or unreachable, reconcile it through its meta as described below.
5. For meta with no pane, reconcile by kind.
...
7. If `state/.afk` is present (away-mode was active before the restart): stay in afk - the supervision extension reloads with this session and honors `state/.afk` to batch escalations; just keep the flag set and follow `skill://afk`.
...
8. Surface only what needs the cap: pending decisions, PRs ready to merge, failures, or needed credentials.
   If there is nothing that needs them, say nothing and resume.
9. The supervision extension is already running (it loaded with this session); there is nothing to arm.
    If `state/.afk` is present, follow `skill://afk` so relevant events remain batched into one digest.
10. Run `sbin/fm lavish-open --recover` to relaunch a steward for every still-open Lavish session this home owns that has no live steward.
