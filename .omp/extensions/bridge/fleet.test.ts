...
import { buildSnapshot, type FleetSnapshot, type HerdrAgent, type ParsedHome, type PendingItem, type PrInfo, render } from "./fleet";
...
// Fixture: firstmate + secondmate mate-a, tasks across both homes, plus a stray pane.
const mainHome: ParsedHome = {
	...
	agents: [
		{ id: "mate-a", meta: { id: "mate-a", pane: "w15:p2", kind: "secondmate", home: "/sm-mate-a", raw: {} }, status: { state: "done", text: "ADOPT" } },
...
const herdr: HerdrAgent[] = [
	...
	{ pane_id: "w15:p2", cwd: "/sm-mate-a", agent_status: "working", name: "mate-a" },
...
check("two mates (firstmate + mate-a)", snap.mates.length === 2);
...
check("mate-a: secondmate, working, load 1", snap.mates[1]?.name === "mate-a" && snap.mates[1]?.role === "secondmate" && snap.mates[1]?.herdrStatus === "working" && snap.mates[1]?.load === 1);
...
check("triage-1: owner is mate-a (mate name, not home dir), project support", byId("triage-1")?.owner === "mate-a" && byId("triage-1")?.project === "support");
...
check("roster: lists the mates", roster.includes("firstmate") && roster.includes("mate-a"));
...
check("tasks: task ids + owners", tasks.includes("feat-x") && tasks.includes("triage-1") && tasks.includes("mate-a"));
...
const stress: FleetSnapshot = {
	...
	mates: [
		...
		{ name: "mate-a", role: "secondmate", herdrStatus: "working", load: 0 },
	],
	tasks: [
		...
		{ id: "q1", state: "queued", owner: "mate-a", note: "blocked-by something-with-a-long-name" },
		{ id: "d1", state: "done", owner: "mate-a", note: "merged", merged: true },
	],
