...
The board is a shared, free-form markdown file that is the primary channel between the captain and the agent.
The captain edits it in nvim; the agent reads the diff and replies by editing the board.
...
The captain may edit anywhere; the agent should not delete captain lines it does not understand.
...
These are captain-facing quick edits over `board.removeLines`/`replaceRange`/`replaceSection`; the agent edits the board through `whiteboard_write` (full replace), so the granular primitives are not exposed as agent tools.
...
Each agent turn directive instructs: read the diff (whiteboard_read), reading the full board only when your context is stale or you explicitly need to re-ground; the extension is loaded once for the named session, so a wall-clock gap alone does not require a full re-read; do the work; append any reply at the bottom of the board (brief; the header already carries the agent id and time, so the agent never restates them); acknowledge by deletion - delete a captain message once read and handled (a message left on the board means unread) and delete the agent's own prior messages once the captain has responded (their reply is the read-ack), so the board holds only the live, unaddressed exchange; maintain `## Working`, keep chat minimal, then call `whiteboard_checkpoint`.

`## Working` is maintained as a next-action queue partitioned by owner: every item is a concrete doable next-action (verb + object) tagged `[agent]` (the loop burns it down) or `[captain]` (a decision, review, or must-read). Gated goals are decomposed to the slice doable now; a truly-blocked item names its unblock and owner; done items leave. A tick then always finds an `[agent]` action or a clean "only `[captain]` items" rest, never a vague stuck. This is a directive-enforced convention, not a parsed schema - the board is never machine-parsed.

Each directive also carries a compact plaintext status header the agent (and the captain, since the turn is displayed) can scan: `tick N`, the agent id, the wall-clock time plus the gap since the previous tick (`+Xm since last tick`), the time since the last human edit (`<$USER> edited: Xm ago` - grows while the captain is away, reads `just now` on a captain-edit turn), the consecutive-turn count, and the previous turn's duration/outcome/result. Each field is on its own line and omitted until it has a value; the machine tick token trails at the end.
...
After `settled`, `needs-decision`, `blocked`, or `error`, the loop rests and waits for the next captain edit.
...
Autonomy is not gated on presence. With autonomy on, a `progress` checkpoint self-continues the loop up to the `max_turns` cap whether the captain is watching this board, working another pane, or away - useful work multiplies regardless of presence. `max_turns` is the sole runaway/cost backstop (raise it to multiply harder); `settled`, `needs-decision`, `blocked`, and `error` rest the loop. A destructive, irreversible, or security-sensitive action is parked on the board as a captain must-read rather than executed autonomously.
...
|-----|---------|-------------|
| `autonomy` | `on` | `on` = self-continue on progress; `off` = one reply per captain edit |
