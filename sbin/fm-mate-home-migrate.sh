...
pane_cwd() {
  local pane=$1
  [ -n "$pane" ] && [ "$pane" != none ] || return 0
  herdr pane get "$pane" 2>/dev/null | fm_json_get result pane cwd
}
...
home_mode() {
  ...
  if "$FM_ROOT/sbin/fm-home-link.sh" "$home" --check >/dev/null 2>&1; then
    printf symlink
    return 0
  fi
  printf invalid
...
resolve_workspace() {
  ...
  workspace_id=$(printf '%s' "$json" | fm_json_get result workspace workspace_id)
...
execute_mate() {
  ...
  if ! "$FM_ROOT/sbin/fm-home-link.sh" "$target" --repair >/dev/null; then
    rollback_from_backup "$id" "$backup" || true
  ...
  update_meta_after_migration "$id" "$target" "$workspace_id" || {
    rollback_from_backup "$id" "$backup" || true
    printf 'execute=rolled-back:meta-write-failed\n'
    return 1
  }
  if ! "$FM_ROOT/sbin/fm-home-link.sh" "$target" --check >/dev/null; then
    rollback_from_backup "$id" "$backup" || true
    printf 'execute=rolled-back:validation-failed\n'
    return 1
  fi
  if [ "$RESPAWN" -eq 1 ]; then
    # Current fm-spawn resumes OMP secondmates with `-c` when their metadata
    # exists. A fresh session must remove that resume signal before spawning;
    # fm-spawn rewrites the metadata after a successful launch.
    if migration_force_fresh_session; then
      rm -f "$STATE/$id.meta"
    fi
    spawn_out=$("$FM_ROOT/sbin/fm-spawn.sh" "$id" --secondmate 2>&1) || {
      printf '%s\n' "$spawn_out" >&2
      rollback_from_backup "$id" "$backup" || true
...
if [ "$EXECUTE" -eq 1 ] && [ "$STOP_LIVE" -ne 1 ]; then
  echo 'result=blocked:stop-live-required'
  exit 1
fi
